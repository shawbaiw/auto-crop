...
- Codex's sandbox is its only control and it governs writes, not tools: `workspace_write` selects `--sandbox workspace-write`, and a grant without it gets `read-only`. The first implementation keyed the sandbox on `run_command` instead, which launched every write-without-shell grant read-only; a real-agent smoke caught a verification run that could not write its required Business Artifact and blocked the company. The corollary is a known limitation: Codex cannot withhold `run_command` from a run that may write, because even `read-only` leaves the shell available. For Codex, withholding `run_command` narrows nothing; Claude Code enforces it by leaving `Bash` out of `--tools`.
