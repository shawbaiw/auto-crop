...
- **The snapshot's files come from the artifact, not from the producer task** (amended after a real-agent smoke). The first implementation copied from the producer task's `artifactWorkspacePath`, which only a department's split subtasks ever carry, and handed over the artifact record alone when there was none — so a verifier of a CEO-planned research report or product brief could not read what it was asked to check. In the smoke every check of a brief came back `not_run`, and the company stopped on an `inconclusive` verdict about a brief that was never the problem. Capture now records the workspace it captured from on the Business Artifact (`deliveryWorkspacePath`), which the runtime writes and the agent cannot state, and the snapshot copies from that — so the files belong to the version being verified, and a task that was never split hands over its files too. A delivery with no recorded workspace fails the handoff by name rather than handing over a record alone: an empty snapshot is not evidence of anything. Artifacts captured before this change carry no workspace and therefore fail closed; they are not migrated (see ADR 0024's note on discarding pre-contract companies).
