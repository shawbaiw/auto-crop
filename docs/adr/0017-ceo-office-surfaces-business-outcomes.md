...
- **Declaration.** The completing agent writes `open_decisions: [{ decisionKind, options, recommended_option_index, rationale }]` into `.auto-crop/business-artifact.json`. The runtime keeps entries whose `decisionKind` is in the enum and that carry more than one option plus a recommendation index into that same array; each becomes a `founder_decision` **Next Step Item** (new `NextStepItemType`, added to the core type and its zod schema) and is projected into a first-class `FounderDecision` object, mirroring the `human_action` → `HumanAction` projection in `ceoAttention.ts`. The source task's artifact stays `deliverable`.
...
- A new `NextStepItemType`, a new `acceptanceProvenance` value, a new `founder_decision` `TaskEventType` (so a founder-decision acceptance is a distinct activity-log entry, not a generic `ceo_review_decision`), and a new `Strategic Decision Kind` enum are added to `packages/core` and its zod schemas — additive, but they widen the persisted vocabulary and cannot be removed without a migration.

## Amendments

### 2026-09-09 — `briefing` added to the Founder Decision declaration (CEO Office Broadcast Timeline, `.scratch/ceo-office-broadcast-timeline/`)

The completing agent's declaration is now `open_decisions: [{ decisionKind, options, recommended_option_index, rationale, briefing }]`. `briefing` is a required founder-facing prose field — what was explored, where the opportunity is, what is differentiated, the monetization angle and why the options exist — carried through the `founder_decision` Next Step Item bag onto the projected `FounderDecision` and its `decision_request` CEO Office Item. It makes the Decision card the post-task substance surface (the pre-execution Task Brief is demoted to a one-line announcement). A known-`decisionKind` entry missing `briefing` is a structural validation failure, on the same footing as a missing `rationale`. Authored in the company canonical locale per ADR 0013; a bare string or an `{ en, zh }` object are both accepted and collapsed to that locale. No new CEO Office Item type, no persisted column (the projection reads it from the completion event's `next_step_items` JSON), and no change to the resolution API or the `Strategic Decision Kind` enum.

### 2026-09-15 — Recommendation is a structural reference, not repeated prose

The recommendation inside `open_decisions` is no longer authored by repeating an option label in a free string. The agent writes `recommended_option_index`, a zero-based index into the sibling `options` array. The runtime derives the display `recommendation` label and `recommended: true` flag from that index.

This closes the class where the model semantically selected an option but failed a cross-field string equality check by shortening, translating, or lightly rewriting the label. Runtime validation now checks only that the index is an integer within `options`. Legacy artifacts whose `recommendation` exactly names an option remain readable for compatibility, but prompts and new artifacts use the structural field.
