...
- Extended by ADR 0021 with the mirror of this decision. Here, runtime-held evidence *confirms* a blocker the agent could not prove. There, it *refutes* one the agent should not have filed: a claim naming a capability the run's Agent Capability Grant actually contained. Same principle — the runtime decides from facts it owns rather than from the agent's account of its environment — applied in the other direction. Capabilities the runtime does not grant, `browser_screenshot` among them, are untouched by it.
