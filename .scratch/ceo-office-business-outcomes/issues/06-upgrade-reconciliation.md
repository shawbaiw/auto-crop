...
- One-shot per company via a `review_reconciliation_v1:<companyId>` key in `runtime_state` (`hasReviewReconciliationRun` / `markReviewReconciliationRun` on the repositories, a pair beside `markTaskAttemptsReset`). The pass runs once per company — on the first scheduler tick or company-state read after the model ships — then every later call short-circuits. This keeps the ADR's "on ship, a one-time reconciliation" literal and stops `GET /state` from carrying a state-changing pass in steady state. The marker is also set for companies created after ship (they provably have no stranded `review` tasks), so the pass only ever does work for the pre-existing migration population. Bump the `v1` suffix to force a re-run if the deterministic conditions change.
- Still safe to run more than once before the marker is set (crash mid-pass, concurrent callers): an accepted task turns `complete` and leaves the `review` filter, a task left in `review` re-fails the same check, so nothing is accepted twice.
