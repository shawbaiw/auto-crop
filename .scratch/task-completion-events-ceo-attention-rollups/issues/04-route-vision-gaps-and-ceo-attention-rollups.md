# 04: Route Vision Gaps And CEO Attention Rollups

**What to build:** Next Step Routing should project Vision Gaps and CEO Attention Rollups from Task Completion Events. Ordinary completions should remain visible in history without interrupting CEO Office, while decision points, cross-department impact, blocking Vision Gaps, strategic Vision Gaps, and executive summaries should roll up by Founder Vision, objective, or dependency chain.

Blocked by: 03: Accept Structured Next Step Items From Business Artifacts.

Status: resolved

- [x] Blocking and strategic Vision Gaps appear in CEO Attention Rollups.
- [x] Informational Vision Gaps remain visible without becoming interruptive attention.
- [x] Decision points and cross-department impacts appear in CEO Attention Rollups.
- [x] Ordinary task completions are recorded without creating CEO Attention Items.
- [x] Rollups preserve owning department, downstream departments, affected tasks, current blocker, and recommended next action.
- [x] Company State Snapshot exposes Vision Gaps and CEO Attention Rollups.
