...
export function buildCeoPrompt(input: BuildCeoPromptInput): string {
  ...
  return [
    ...
    "Every task must also include decomposition: null, or { \"template\": \"define_execute_validate\" }, which hands the task to its department",
    "in three stages: fix the executable slice and the conditions it must meet, build it, then check what was built against exactly those",
    "conditions. Declare it when the task is judged on something it builds and runs rather than on the reasoning it writes down — so its",
    "acceptance conditions are worth settling before the work starts, not after. Otherwise declare null. A task that declares verification",
    "never declares a decomposition.",
    "",
    "```json",
    JSON.stringify(
      {
